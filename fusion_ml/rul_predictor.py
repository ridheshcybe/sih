...
class RULPredictor:
    ...
    def train(
        ...
        2. Build sequences + baseline features.
        ...
        for run_id in run_ids:
            ...
            for _, row in run_df.iterrows():
                ...
                fused = np.array(result["fused_vector"], dtype=np.float32)

                all_fused.append(fused)
                all_rul.append(float(row["remaining_useful_life_sec"]) / 60.0)  # sec → min
                all_cht_max.append(max(frame["cht_c"]))
                all_egt_spread.append(max(frame["egt_c"]) - min(frame["egt_c"]))
                all_vib.append(frame["vibration_rms_g"])
                all_is_anomaly.append(int(row.get("is_anomaly", 0)))

        fused_arr = np.array(all_fused)
        rul_arr = np.array(all_rul)
        cht_max_arr = np.array(all_cht_max)
        egt_spread_arr = np.array(all_egt_spread)
        vib_arr = np.array(all_vib)
        is_anom = np.array(all_is_anomaly)

        print(f"  Fused vectors: {fused_arr.shape}")
        print(f"  RUL range: {rul_arr.min():.1f} – {rul_arr.max():.1f} min")

        # ── Step 3: Build sequences ──
        # Only use sequences where the window ends at an anomalous point
        # (RUL is meaningful only during degradation)
        X_seq, X_base, y_rul = self.build_sequences(
            fused_arr, rul_arr, cht_max_arr, egt_spread_arr, vib_arr,
        )
